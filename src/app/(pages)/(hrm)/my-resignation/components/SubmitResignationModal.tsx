...
const SubmitResignationModal: React.FC<Props> = ({ onClose, onSuccess }) => {
      ...
      <div style={{
        ...
        <div style={{ padding: "28px 28px 0" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
            <div style={{ display: "flex", alignItems: "center", gap: "14px" }}>
              <div style={{ width: "48px", height: "48px", borderRadius: "14px", background: "linear-gradient(135deg, #6366f1, #4f46e5)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "22px", boxShadow: "0 4px 12px rgba(99,102,241,0.3)" }}>📝</div>
              <div>
                <h2 style={{ margin: 0, fontSize: "20px", fontWeight: 700, color: "#1e293b" }}>Submit Resignation</h2>
                <p style={{ margin: "2px 0 0", color: "#64748b", fontSize: "13px" }}>Notice period decided by HR</p>
              </div>
            </div>
        ...
        <form onSubmit={handleSubmit} style={{ padding: "24px 28px 28px" }}>
          ...
          <div style={{ marginBottom: "16px" }}>
            ...
            <textarea name="reason" value={form.reason} onChange={handleChange} rows={3} placeholder="Why are you resigning..." style={{ ...inputStyle, resize: "vertical", lineHeight: 1.5 }} onFocus={(e) => (e.target.style.borderColor = "#4f46e5")} onBlur={(e) => (e.target.style.borderColor = "#e5e7eb")} />
          </div>

          {/* Handover To */}
          <div style={{ marginBottom: "24px" }}>
            <label style={{ fontWeight: 600, fontSize: "13px", color: "#374151", display: "block", marginBottom: "6px" }}>Handover To</label>
            <input type="text" name="handoverTo" value={form.handoverTo} onChange={handleChange} placeholder="Name of person taking over..." style={inputStyle} onFocus={(e) => (e.target.style.borderColor = "#4f46e5")} onBlur={(e) => (e.target.style.borderColor = "#e5e7eb")} />
          </div>
...
export default SubmitResignationModal;
