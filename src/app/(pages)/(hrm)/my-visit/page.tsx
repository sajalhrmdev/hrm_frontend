...
const MyVisitPage = () => {
  ...
  const fetchMyVisits = async () => {
    try {
      setLoading(true);
      const res = await axiosInstance.get(`/visit/my?page=${page}&limit=20`);
  ...
  const handleStatusChange = async (id: number, status: string) => {
    try {
      setUpdatingId(id);
      await axiosInstance.patch(`/visit/my/${id}/status`, { status });
      await fetchMyVisits();
    } catch (err) {
      console.error(err);
    } finally {
        ...
        <div className="content">
          <div className="d-md-flex d-block align-items-center justify-content-between page-breadcrumb mb-3">
            <div className="my-auto mb-2">
              <h2 className="mb-1">My Visits</h2>
              <nav>
            ...
            <div className="card-body p-0">
              {loading ? (
                ...
                <div className="text-center py-5">
                  <i className="ti ti-map-pin-off fs-1 text-muted d-block mb-2" />
                  <p className="text-muted mb-0">No visits scheduled.</p>
                </div>
              ) : (
