...
const EmployeeListComponent = () => {
  ...
  const datas=empList
  const [data] = useState<Employee[]>(employee_list_details);
  const [visibleData, setVisibleData] = useState<Employee[]>(
    data.slice(0, PAGE_SIZE)
  );
  const [hasMore, setHasMore] = useState<boolean>(data.length > PAGE_SIZE);
        ...
        <div className="d-flex align-items-center">
          ...
          >
            <ImageWithBasePath
              ...
              alt={`${record.user?.name}'s profile image`} // alt is now required and descriptive
            />
          </Link>
          <div className="ms-2">
  ...
  const handleLoadMore = useCallback(() => {
    ...
    setTimeout(() => {
      setVisibleData((prev) => {
        const next = data.slice(prev.length, prev.length + PAGE_SIZE);
        ...
        <div className="content">
          ...
          <div className="card">
            ...
            <div className="card-body p-0">
             <Table dataSource={datas} columns={columns} Selection={true} />
            </div>
          </div>
        </div>
